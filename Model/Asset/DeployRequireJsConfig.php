...
use Magento\Deploy\Service\DeployRequireJsConfig as CoreDeployRequireJsConfig;

/**
 * A service for deploying requirejs configuration files.
 */
class DeployRequireJsConfig extends CoreDeployRequireJsConfig
{
    /**
     * @var ListInterface
     */
    ...
    public function deploy($areaCode, $themePath, $localeCode)
    {
        ...
        // Skip minification for the first run
        if ($fileManager instanceof \Mageproxy\Connector\Model\Asset\FileManager) {
            $fileManager->setSkipMinification(true);
            if ($fileManager->isConfigFileRelativePathMinified()) {
                $fileManager->createRequireJsConfigAsset();
            }
            if ($fileManager->isMinResolverRelativePathMinified()) {
                $fileManager->createMinResolverAsset();
            }
            $fileManager->setSkipMinification(false);
        }
